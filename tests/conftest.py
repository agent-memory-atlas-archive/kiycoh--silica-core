...
def _restore_tools_registry() -> None:
    ...
    for name in set(TOOLS) - set(snapshot):
        if not getattr(TOOLS[name].fn, "__module__", "").startswith("silica_core"):
            del TOOLS[name]
...
def _isolate_silica_home(tmp_path_factory, monkeypatch: pytest.MonkeyPatch):
    ...
    import silica_core.kernel.recall.paths as paths_mod
...
def _reset_manifest_cache() -> None:
    import silica_core.kernel.vault_manifest as manifest_mod
...
def _clear_store_singletons() -> None:
    ...
    import silica_core.kernel.recall.lexical as lex
...
def _isolate_vault_path(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import silica_core.config as config_mod
...
def tmp_vault(tmp_path, monkeypatch):
    ...
    import silica_core.config
    import silica_core.driver

    vault_dir = tmp_path / "vault"
    vault_dir.mkdir()
    monkeypatch.setattr(silica_core.config.CONFIG, "vault_path", str(vault_dir))
    silica_core.driver._driver = None
