...
from silica_core.config import CONFIG, SILICA_HOME
...
def is_source_leaf(path: str) -> bool:
    ...
    if norm.startswith(SOURCES_DIR + "/"):
        return True
    try:
        from silica_core.kernel.vault_manifest import in_write_dir
...
def looks_like_code(root, sample_max: int = 400) -> bool:
    ...
    """
    from silica_core.kernel.code.codeast.base import BARE_LANGUAGES, EXTENSION_MAP
...
def is_inbox_path(path: str) -> bool:
    ...
    """
    from silica_core.config import CONFIG
    from silica_core.kernel.vault_manifest import active_inbox_dir, in_write_dir
