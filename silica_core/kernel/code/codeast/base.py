...
def extract_skeleton(source: str, language: str, path: str = "") -> ModuleSkeleton:
    ...
    # silica_core/ui/web/static/) raises RecursionError. Escaping here takes the
    ...
    try:
        ...
        if language == "java":
            from silica_core.kernel.code.codeast import java as _java
            return _java.extract(root, src, path=path, language=language)
        if language in ("c", "cpp"):
            from silica_core.kernel.code.codeast import c as _c
        ...
        if language == "python":
            ...
            from silica_core.kernel.code.codeast import ts as _ts
